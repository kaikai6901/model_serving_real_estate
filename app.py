...
def update():
    try:
        ...
        another_model = None
        if os.path.exists(model_local_path):
            another_model = joblib.load(model_local_path)
        else:
            create_run_folder('./model/', run_id)
            mlflow_model_path = 'runs:/' + run_id + '/model'
            another_model = mlflow.sklearn.load_model(mlflow_model_path)
            joblib.dump(another_model, model_local_path)
        model = another_model
...
def get_all_projects():
    try:
        projects = list(collection.find({'run_id': run_id}, {'_id': 0, 'name': 1, 'project_id': 1, 'loc': 1}))
